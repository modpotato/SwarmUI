...
Projects in C# normally have a `.csproj` file in their root. SwarmUI extensions are no different! Here's a fully working csproj you can start with:
```xml
...
- Notice how by default, almost everything in the configuration comes from the [handy dandy SwarmUI built-in extension properties source](<https://github.com/mcmonkeyprojects/SwarmUI/blob/master/src/SwarmUI.extension.props>)
    - You can customize or replace the values it defines at will.
    - Note that if you mess with the dependencies or framework target, things may go weird in the compile stage. Generally trust the import to handle the basics.
- If you do not have a csproj, the above template will be autogenerated. *This is mostly for legacy compatible with extensions the predate the modern csproj system.*
- If you have multiple csproj files in your extension folder root, include the word `Extension` in the main one, for example `MyCustomExtension.csproj`, and it will take priority.
